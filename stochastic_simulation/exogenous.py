...
parser.add_argument('-g', '--g', default=1e6, type=float)
parser.add_argument('-k', '--kappa', default=1e4, type=float)
parser.add_argument('-z1', '--Z1', default=5e3, type=float)
parser.add_argument('-z2', '--Z2', default=1, type=float)
parser.add_argument('-ga', '--gamma', default=5e5, type=float)
parser.add_argument('-D', '--diffusion', default=1, type=float)
...
parser.add_argument('-n', '--num_reactions', default=5000000, type=float)
parser.add_argument('-L', '--num_sites', default=300, type=float)
parser.add_argument('-o', '--outfile', default='exogenous.p', type=str)
...
class ConsumptionBirth(object):
    def __init__(self, mother, food, rate, L, l_psi, Z):
        self.mother = mother
        self.food = food
        self.stoich = {self.food: -1, self.mother: 1}
...
class ConsumerMutation(object):
    def __init__(self, mother, rate, L, levo):
        self.mother = mother
        self.mutants = list((np.arange(mother-levo, mother+levo+1).astype(int)-L)%L+L)
        self.daughter = self.mutants[0]
        self.stoich = {self.mother:-1, self.daughter: 1}
        self.rate_const = rate
        self.reactants = [self.mother]
        self.type='mutation'
...
print('Effective diffusion constant = '+str(args.diffusion/2))
...
print('created resource birth reactions...')
    
# Resource deaths
rate = args.g/args.kappa
for i in range(L):
    reactions[L+i] = ResourceDeath(i, rate, L)
print('created resource death reactions...')

# Consumer deaths
rate = args.gamma
for i in range(L):
    reactions[2*L+i] = ConsumerDeath(L+i, rate, L)
print('created consumer death reactions...')

# Consumer mutations
rate = args.diffusion
for i in range(L):
    reactions[3*L+i] = ConsumerMutation(L+i, rate, L, args.levo)

print('created consumer mutation reactions...')
...
for i in range(L):
    for j in range(L):
        reactions[4*L+temp] = Consumption(L+i, j, rate, L, l_phi, Z)
        temp+=1
print('created consumption reactions...')
...
for i in range(L):
    for j in range(L):
        reactions[4*L+L**2+temp] = ConsumptionBirth(L+i, j, rate, L, l_psi, Z)
        temp+=1
print('created consumption+birth reactions...Done creating reactions!')
...
print('Number of reactions executed:')
# run simulation
